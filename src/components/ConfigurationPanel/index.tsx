...
function CategoriesManager(props: any) {
	const { isLoading, error, data } = useQuery("categories_query", async () => {
	...
	function CategoryLine(c_props: any) {
		...
		const handleDelete = async (event: MouseEvent<HTMLButtonElement>) => {
			const IdToDelete = Number(event.currentTarget.parentElement?.getAttribute("data-category-id"));
			...
			.then(_ => document.querySelector(`.category_line[data-category-id='${IdToDelete}']`)?.remove())
			.catch((err => console.error(err)));
		};
		const handleUpdate = async (event: MouseEvent<HTMLButtonElement>) => {
			const clickedElement = event.currentTarget;
			const idToUpdate = Number(event.currentTarget.parentElement?.getAttribute("data-category-id"));
		...
		return <div data-category-id={c_props['categoryId']} className="card__row">
			<input key={"input" + c_props['tabIndex'] + 1 + c_props['categoryId']} tabIndex={c_props['tabIndex'] + 1} type='text' className='category_name card__input' value={text} title={text} onChange={handleInputChange}></input>
			<button className='remove_category card__input card__input__button' title='Zmiłuj się' onClick={handleDelete}> Usuń </button>
			<button className='confirm_new_name card__input card__input__button' onClick={handleUpdate} title='Zatwierdź nową nazwę' data-value={text}> Zatwierdź </button>
		</div>
	}
	return (
		<div id='category_manager'>
            ...
            <div id='categories_container' className='card__container'>
			    {data.map((el: any, index: number) => <CategoryLine tabIndex={index} categoryName={el['name']} categoryId={el['id']} key={el['id'] + index + el['name']}></CategoryLine>)}
			</div>
            <button className='card__input card__input__button' id='btn_confirm_all'> Zatwierdź wszystkie </button>
		</div>
	)
}
...
function Reset(props: any) {
	...
	return (
		<form id='reset' className='card'>
			...
			<input type='text' name='newDatabase' className='card__input' id='newDatabase' />
			<button title='Stworzy nową bazę danych' className='card__input card__input__button '> Reset </button>
...
function ShortcutList(props: any) {
    return (<div id='shortcutManager' className='card'>
